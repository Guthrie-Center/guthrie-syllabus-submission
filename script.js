...
function renderCertificationChecklist(selected=[]){
  ...
  (options.certificationOptions || []).forEach(cert => {
    ...
    label.innerHTML = `
      <input type="checkbox" name="certifications" value="${escapeHtml(cert)}" ${checked}>
...
function init(){
  ...
  populateSelect(
    teacherSelect,
    unique(records.map(r => r.teacher)),
    "Select teacher..."
  );

  populateSelect(
    document.getElementById("testingMonth"),
    options.months || [],
    "Select month if applicable..."
  );

  populateSelect(
    document.getElementById("ctso"),
    options.ctsoOptions || [],
    "Select CTSO..."
  );

  populateSelect(
    document.getElementById("tutorialTime"),
    options.tutorialDays || [],
    "Select tutorial day..."
  );
...
teacherSelect.addEventListener("change", () => {
  ...
  const courseLabels = teacherRecords.map(
    r => `${r.course} — ${r.semester} — ${r.blocks}`
  );
...
courseSelect.addEventListener("change", () => {
  ...
  selectedRecord = teacherRecords.find(
    r => `${r.course} — ${r.semester} — ${r.blocks}` === label
  );
  ...
  renderUnits(
    selectedRecord.semester === "Year" ||
    selectedRecord.semester === "Yearlong"
      ? 6
      : 4
  );
...
document.getElementById("noProfessionalDress")
...
function renderKnownInfo(r){
  ...
  knownInfo.innerHTML = `
    ...
    <div class="facts">
      ...
      <div class="fact">
        <b>Course</b>${escapeHtml(r.course)}
      </div>

      <div class="fact">
        <b>Teacher</b>${escapeHtml(r.teacher)}
      </div>

      <div class="fact">
        <b>Email</b>${escapeHtml(r.email || "TBA")}
      </div>

      <div class="fact">
        <b>Term</b>${escapeHtml(r.semester)}
      </div>

      <div class="fact">
        <b>Blocks</b>${escapeHtml(r.blocks)}
      </div>

      <div class="fact">
        <b>Conference</b>${escapeHtml(r.conference || "TBA")}
      </div>

      <div class="fact">
        <b>Program</b>${escapeHtml(r.program)}
      </div>

      <div class="fact">
        <b>Room</b>${escapeHtml(r.room || "TBA")}
      </div>

      <div class="fact">
        <b>Suggested CTSO</b>${escapeHtml(r.ctso || "TBA")}
      </div>
...
function prefillProgramFields(r){
  ...
  if(
    r.ctso &&
    [...ctsoSelect.options].some(o => o.value === r.ctso)
  ){
  ...
  for(let i=1;i<=count;i++){
    ...
    div.innerHTML = `
      ...
      <label>
        ...
        <input
          name="unit${i}Title"
          placeholder="Example: Patient Assessment Skills"
        >
      </label>
      ...
      <label>
        ...
        <textarea
          name="unit${i}Topics"
          rows="2"
          placeholder="Brief summary of major concepts or skills."
        ></textarea>
      </label>
      ...
      <label>
        ...
        <textarea
          name="unit${i}HandsOn"
          rows="2"
          placeholder="Labs, projects, simulations, field experiences, or applied learning."
        ></textarea>
      </label>
      ...
      <label>
        ...
        <textarea
          name="unit${i}Assessment"
          rows="2"
          placeholder="How understanding will be measured."
        ></textarea>
      </label>
    `;
...
function draftKey(){
  return selectedRecord
    ? `guthrie-syllabus-draft-${selectedRecord.id}`
    : null;
}
...
function saveDraft(){
  ...
  statusMessage.textContent =
    "Draft saved on this computer/browser.";
...
function loadDraft(){
  ...
  if(raw){
    ...
    statusMessage.textContent =
      "A saved draft was loaded from this computer/browser.";
...
form.addEventListener("submit", async (e) => {
  ...
  try{
    ...
    alert(
      "Submitted for review. The form has been cleared for the next entry."
    );
    ...
    statusMessage.textContent =
      "Submission may not have completed. Save your draft and contact Joe if this continues.";
...
init();  prefillProgramFields(selectedRecord);
  loadDraft();
  form.classList.remove("hidden");
});
